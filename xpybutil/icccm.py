...
def get_wm_name_unchecked(window):
    return util.PropertyCookie(util.get_property_unchecked(window, atoms.WM_NAME))
...
def get_wm_icon_name_unchecked(window):
    return util.PropertyCookie(util.get_property_unchecked(window, atoms.WM_ICON_NAME))
...
def get_wm_normal_hints_unchecked(window):
    return NormalHintsCookie(util.get_property_unchecked(window, atoms.WM_NORMAL_HINTS))


def _pack_normal_hints(
    flags,
    x,
    y,
    width,
...
def set_wm_normal_hints_checked(
    window,
    flags,
    x=0,
    y=0,
...
def set_wm_hints(
    ...
    return c.core.ChangeProperty(
        xproto.PropMode.Replace, window, atoms.WM_HINTS, atoms.WM_HINTS, 32, 9, packed
    )


def set_wm_hints_checked(
    window,
    flags,
    input=1,
    initial_state=State.Normal,
...
def get_wm_class_unchecked(window):
    return util.PropertyCookie(util.get_property_unchecked(window, atoms.WM_CLASS))
...
def get_wm_transient_for(window):
    return util.PropertyCookie(util.get_property(window, atoms.WM_TRANSIENT_FOR))
...
def get_wm_protocols_unchecked(window):
    return util.PropertyCookie(util.get_property_unchecked(window, "WM_PROTOCOLS"))
...
def get_wm_client_machine(window):
    return util.PropertyCookie(util.get_property(window, atoms.WM_CLIENT_MACHINE))
...
def get_icon_size_unchecked(window):
    return IconSizeCookie(util.get_property_unchecked(window, atoms.WM_ICON_SIZE))
...
def set_icon_size(
    ...
    packed = struct.pack(
        "I" * 6, min_width, min_height, max_width, max_height, width_inc, height_inc
    )
    return c.core.ChangeProperty(
        xproto.PropMode.Replace,
        window,
