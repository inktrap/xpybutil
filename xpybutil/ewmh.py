...
def get_supported_unchecked():
    return util.PropertyCookie(util.get_property_unchecked(root, "_NET_SUPPORTED"))
...
def set_supported_checked(atoms):
    packed = struct.pack("I" * len(atoms), *atoms)
    return c.core.ChangePropertyChecked(
        xproto.PropMode.Replace,
        root,
        atom("_NET_SUPPORTED"),
        ATOM,
...
def get_client_list_unchecked():
    return util.PropertyCookie(util.get_property_unchecked(root, "_NET_CLIENT_LIST"))
...
def get_client_list_stacking():
    ...
    :rtype:         util.PropertyCookie (ATOM[]/32)
    """
    return util.PropertyCookie(util.get_property(root, "_NET_CLIENT_LIST_STACKING"))
...
def get_desktop_geometry():
    ...
    """
    return DesktopGeometryCookie(util.get_property(root, "_NET_DESKTOP_GEOMETRY"))
...
def get_desktop_viewport():
    ...
    """
    return DesktopViewportCookie(util.get_property(root, "_NET_DESKTOP_VIEWPORT"))
...
def get_current_desktop():
    ...
    :rtype:         util.PropertyCookieSingle (CARDINAL/32)
    """
    return util.PropertyCookieSingle(util.get_property(root, "_NET_CURRENT_DESKTOP"))
...
def request_current_desktop(desktop_number, timestamp=xproto.Time.CurrentTime):
    """
...
def request_current_desktop_checked(desktop_number, timestamp=xproto.Time.CurrentTime):
    return revent_checked(root, "_NET_CURRENT_DESKTOP", desktop_number, timestamp)
...
def get_active_window():
    ...
    """
    return util.PropertyCookieSingle(util.get_property(root, "_NET_ACTIVE_WINDOW"))
...
def set_active_window(active):
    ...
    return c.core.ChangeProperty(
        xproto.PropMode.Replace, root, atom("_NET_ACTIVE_WINDOW"), WINDOW, 32, 1, packed
    )
...
def request_active_window(
    active, source=1, timestamp=xproto.Time.CurrentTime, current=0
):
    """
...
def request_active_window_checked(
    active, source=1, timestamp=xproto.Time.CurrentTime, current=0
):
    return revent_checked(active, "_NET_ACTIVE_WINDOW", source, timestamp, current)
...
class WorkareaCookie(util.PropertyCookie):
    def reply(self):
        ...
        for i in range(0, len(v), 4):
            ret.append(
                {"x": v[i], "y": v[i + 1], "width": v[i + 2], "height": v[i + 3]}
            )
...
def set_desktop_layout(
    orientation, columns, rows, starting_corner=StartingCorner.TopLeft
):
    """
...
def set_desktop_layout_checked(
    orientation, columns, rows, starting_corner=StartingCorner.TopLeft
):
    packed = struct.pack("IIII", orientation, columns, rows, starting_corner)
    return c.core.ChangePropertyChecked(
...
def request_close_window(window, timestamp=xproto.Time.CurrentTime, source=1):
    """
...
def request_close_window_checked(window, timestamp=xproto.Time.CurrentTime, source=1):
...
def request_moveresize_window(
    ...
    return revent(
        window, "_NET_MOVERESIZE_WINDOW", flags, x or 0, y or 0, width or 0, height or 0
    )


def request_moveresize_window_checked(
    window,
    x=None,
    y=None,
    width=None,
    ...
    return revent_checked(window, "_NET_MOVERESIZE_WINDOW", flags, x, y, width, height)


# _NET_WM_MOVERESIZE


def request_wm_moveresize(window, direction, x_root=0, y_root=0, button=0, source=1):
    """
    ...
    """
    return revent(
        window, "_NET_WM_MOVERESIZE", x_root, y_root, direction, button, source
    )


def request_wm_moveresize_checked(
    window, direction, x_root=0, y_root=0, button=0, source=1
):
    return revent_checked(
        window, "_NET_WM_MOVERESIZE", x_root, y_root, direction, button, source
    )
...
def request_restack_window(
    window, stack_mode=xproto.StackMode.Above, sibling=0, source=1
):
    """
...
def request_restack_window_checked(
    window, stack_mode=xproto.StackMode.Above, sibling=0, source=2
):
    return revent_checked(window, "_NET_RESTACK_WINDOW", source, sibling, stack_mode)
...
def get_wm_name_unchecked(window):
    return util.PropertyCookie(util.get_property_unchecked(window, "_NET_WM_NAME"))
...
def get_wm_visible_name(window):
    ...
    :rtype:         util.PropertyCookie (UTF8_STRING)
    """
    return util.PropertyCookie(util.get_property(window, "_NET_WM_VISIBLE_NAME"))
...
def get_wm_visible_icon_name(window):
    ...
    :rtype:         util.PropertyCookie (UTF8_STRING)
    """
    return util.PropertyCookie(util.get_property(window, "_NET_WM_VISIBLE_ICON_NAME"))
...
def get_wm_window_opacity(window):
    ...
    :rtype:         util.PropertyCookieSingle (CARDINAL/32)
    """
    return OpacityCookieSingle(util.get_property(window, "_NET_WM_WINDOW_OPACITY"))
...
def get_wm_desktop(window):
    ...
    :rtype:         util.PropertyCookieSingle (CARDINAL/32)
    """
    return util.PropertyCookieSingle(util.get_property(window, "_NET_WM_DESKTOP"))
...
def get_wm_state_unchecked(window):
    return util.PropertyCookie(util.get_property_unchecked(window, "_NET_WM_STATE"))
...
def request_wm_state_checked(window, action, first, second=0, source=1):
    return revent_checked(window, "_NET_WM_STATE", action, first, second, source)
...
def get_wm_allowed_actions(window):
    ...
    :rtype:         util.PropertyCookie (ATOM[]/32)
    """
    return util.PropertyCookie(util.get_property(window, "_NET_WM_ALLOWED_ACTIONS"))
...
def set_wm_strut(window, left, right, top, bottom):
    ...
    return c.core.ChangeProperty(
        xproto.PropMode.Replace, window, atom("_NET_WM_STRUT"), CARDINAL, 32, 4, packed
    )
...
def get_wm_strut_partial(window):
    ...
    """
    return StrutPartialCookie(util.get_property(window, "_NET_WM_STRUT_PARTIAL"))
...
def set_wm_strut_partial_checked(
    window,
    left,
    right,
    top,
...
def get_wm_icon_geometry(window):
    ...
    """
    return IconGeometryCookie(util.get_property(window, "_NET_WM_ICON_GEOMETRY"))
...
class IconCookie(util.PropertyCookie):
    def reply(self):
        ...
        while start < len(v):
            ...
            ret.append(
                {"width": w, "height": h, "data": v[start + 2 : start + upto + 2]}
            )
...
def set_wm_pid(window, pid):
    ...
    return c.core.ChangeProperty(
        xproto.PropMode.Replace, window, atom("_NET_WM_PID"), CARDINAL, 32, 1, packed
    )
...
def get_wm_handled_icons(window):
    ...
    :rtype:         util.PropertyCookieSingle (CARDINAL/32)
    """
    return util.PropertyCookieSingle(util.get_property(window, "_NET_WM_HANDLED_ICONS"))
...
def get_wm_user_time(window):
    ...
    :rtype:         util.PropertyCookieSingle (CARDINAL/32)
    """
    return util.PropertyCookieSingle(util.get_property(window, "_NET_WM_USER_TIME"))
...
def request_wm_ping(window, response=False, timestamp=xproto.Time.CurrentTime):
    """
    ...
    """
    return revent(
        window if not response else root(c),
        "WM_PROTOCOLS",
        atom("_NET_WM_PING"),
        timestamp,
        window,
...
def request_wm_ping_checked(window, response=False, timestamp=xproto.Time.CurrentTime):
    return revent_checked(
        window if not response else root(c),
        "WM_PROTOCOLS",
        atom("_NET_WM_PING"),
...
def request_wm_sync_request(window, req_num, timestamp=xproto.Time.CurrentTime):
    """
    ...
    return revent(
        window, "WM_PROTOCOLS", atom("_NET_WM_SYNC_REQUEST"), timestamp, low, high
    )


def request_wm_sync_request_checked(window, req_num, timestamp=xproto.Time.CurrentTime):
    high = req_num >> 32
    low = (high << 32) ^ req_num
...
class FullscreenMonitorsCookie(util.PropertyCookie):
    def reply(self):
        ...
        return {"top": v[0], "bottom": v[1], "left": v[2], "right": v[3]}
...
def request_wm_fullscreen_monitors(window, top, bottom, left, right, source=1):
    """
    ...
    """
    return revent(
        window, "_NET_WM_FULLSCREEN_MONITORS", top, bottom, left, right, source
    )


def request_wm_fullscreen_monitors_checked(window, top, bottom, left, right, source=1):
    return revent_checked(
        window, "_NET_WM_FULLSCREEN_MONITORS", top, bottom, left, right, source
    )
